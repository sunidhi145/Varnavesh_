...
function getSeedProductBySlug(slug) {
    return seedProducts.find((product) => product.slug === slug);
}
function withSeedDefaults(product) {
...
function mergeWithSeedProducts(products, productType) {
...
export async function fetchProducts(productType) {
    ...
    try {
      ...
      `, {
        ...
        const products = result.records.map((record) => mapProduct(record.get("product")));
...
export async function createCustomDesignRequest(requestId, input, files) {
    if (input.honeypot) {
        throw new ApiError("Spam submission rejected.");
    }
    const driver = getNeo4jDriver();
...
export async function seedDatabase(products) {
    const driver = getNeo4jDriver();
    const session = driver.session({ database: serverConfig.neo4j.database });
